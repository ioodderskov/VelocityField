...
sigma_phi, pcov = curve_fit(phi, bin_centers,fractions_in_bins,p0=[500])
...
ax1.plot(x,fitted_distribution_phi, 'b', lw=2,
    label='Fit to phi: sigma=%0.1f' %(sigma_phi))
plt.legend()



plt.xlabel('$v_{||}$ [km/s]',fontsize=16)
...
print integrate.quad(lambda x: phi(x,sigma_phi),-3000,3000)

plt.savefig(path+'pairwise_velocity_distribution_phi.pdf')
